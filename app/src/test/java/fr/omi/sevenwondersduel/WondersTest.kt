...
import fr.omi.sevenwondersduel.BuildingType.MANUFACTURE
...
class WondersTest {
    ...
    fun the_statue_of_zeus_destroys_an_opponent_raw_material_building() {
        var game = SevenWondersDuel(structure = createStructure(AGE_II, listOf(DISPENSARY)), players = Pair(
    ...
    fun the_great_library_allows_to_choose_a_random_progress_token_between_3_remaining() {
        ...
        game = SevenWondersDuel(structure = createStructure(AGE_I, listOf(CLAY_POOL)),
    ...
    fun the_great_lighthouse_produces_any_raw_good_once_built() {
        var game = SevenWondersDuel(structure = createStructure(AGE_II, listOf(FORUM)), players = Pair(
        ...
        game = SevenWondersDuel(structure = createStructure(AGE_II, listOf(FORUM)), players = Pair(
    ...
    fun piraeus_produces_any_manufactured_good_once_built() {
        var game = SevenWondersDuel(structure = createStructure(AGE_III, listOf(CHAMBER_OF_COMMERCE)), players = Pair(
                Player(coins = 4, wonders = listOf(BuildableWonder(PIRAEUS))),
                Player()
        ))
        game = game.build(CHAMBER_OF_COMMERCE)
        assertThat(game.players.first.coins).isEqualTo(0)
        game = SevenWondersDuel(structure = createStructure(AGE_III, listOf(CHAMBER_OF_COMMERCE)), players = Pair(
    ...
    fun the_mausoleum_let_me_build_any_discarded_card_for_free() {
        var game = SevenWondersDuel(structure = createStructure(AGE_II, listOf(DISPENSARY, DRYING_ROOM)), players = Pair(
    ...
    fun the_mausoleum_does_not_let_me_build_anything() {
        val game = SevenWondersDuel(structure = createStructure(AGE_II, listOf(DISPENSARY, DRYING_ROOM)), players = Pair(
                Player(coins = 10, wonders = listOf(BuildableWonder(THE_MAUSOLEUM))),
                Player()), discardedCards = listOf(WALLS, TRIBUNAL))

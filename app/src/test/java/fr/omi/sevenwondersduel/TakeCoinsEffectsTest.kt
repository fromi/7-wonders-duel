...
class TakeCoinsEffectsTest {
    ...
    private val sampleAge1Structure = Game.createStructure(Age.AGE_I, listOf(
    ...
    private val sampleAge2Structure = Game.createStructure(Age.AGE_II, listOf(
    ...
    fun tavern_gives_4_coins() {
        var game = Game(structure = sampleAge1Structure, players = Pair(Player(coins = 0), Player(coins = 7)))
    ...
    fun brewery_gives_6_coins() {
        var game = Game(structure = sampleAge2Structure, players = Pair(Player(coins = 0), Player(coins = 7)))
    ...
    fun chamber_of_commerce_gives_3_coins_per_manufacture_building_I_have() {
        var game = Game(structure = Game.createStructure(Age.AGE_III, listOf(CHAMBER_OF_COMMERCE)),
    ...
    fun port_gives_2_coins_per_raw_material_building_I_have() {
        var game = Game(structure = Game.createStructure(Age.AGE_III, listOf(PORT)), players = Pair(
    ...
    fun armory_gives_1_coins_per_military_building_I_have() {
        var game = Game(structure = Game.createStructure(Age.AGE_III, listOf(ARMORY)), players = Pair(
    ...
    fun lighthouse_gives_1_coins_per_commercial_building_I_have_including_it() {
        var game = Game(structure = Game.createStructure(Age.AGE_III, listOf(LIGHTHOUSE)), players = Pair(
    ...
    fun arena_gives_2_coins_per_wonder_I_built() {
        var game = Game(structure = Game.createStructure(Age.AGE_III, listOf(ARENA)), players = Pair(
    ...
    fun agriculture_gives_6_coins() {
        var game = Game(pendingActions = listOf(ChooseProgressToken(setOf(AGRICULTURE))),
    ...
    fun urbanism_gives_6_coins() {
        var game = Game(pendingActions = listOf(ChooseProgressToken(setOf(URBANISM))),
                players = Pair(Player(coins = 0), Player()))

...
class ProgressTokensTest {
    ...
    fun architecture_gives_a_reduction_on_wonders_construction() {
        var game = Game(structure = Game.createStructure(AGE_III, listOf(PALACE)), players = Pair(
    ...
    fun with_the_economy_I_gain_the_money_spent_by_my_opponent_on_resources() {
        val game = Game(structure = Game.createStructure(AGE_III, listOf(TOWN_HALL, COURTHOUSE)), players = Pair(
    ...
    fun law_gives_a_scientific_symbol() {
        val game = Game(players = Pair(Player(progressTokens = setOf(LAW)), Player()))
    ...
    fun masonry_gives_a_reduction_on_civilian_building_construction() {
        val game = Game(structure = Game.createStructure(AGE_III, listOf(PALACE, ACADEMY)), players = Pair(
    ...
    fun strategy_give_extra_shield_for_military_buildings() {
        var game = Game(structure = Game.createStructure(AGE_II, listOf(ARCHERY_RANGE)),
    ...
    fun theology_give_play_again_effect_to_all_my_wonders() {
        var game = Game(structure = Game.createStructure(AGE_III, listOf(PALACE, OBSERVATORY)), players = Pair(
    ...
    fun urbanism_gives_4_coin_for_each_chained_building() {
        val game = Game(structure = Game.createStructure(AGE_II, listOf(BARRACKS, TEMPLE, AQUEDUCT)),

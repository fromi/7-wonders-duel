...
class BuildingConstructionTest {
    ...
    fun build_with_resources() {
        var game = Game(players = Pair(Player(buildings = setOf(QUARRY)), Player()),
    ...
    fun trade_resource_increases_when_opponent_produces() {
        var game = Game(structure = sampleAge1Structure, players = Pair(
        ...
        game = Game(structure = sampleAge1Structure, players = Pair(
                first = Player(coins = 7),
    ...
    fun cannot_build_if_not_enough_coins() {
        val game = Game(players = Pair(Player(coins = 1), Player()), structure = sampleAge1Structure)
        game.build(BATHS)
    }
}
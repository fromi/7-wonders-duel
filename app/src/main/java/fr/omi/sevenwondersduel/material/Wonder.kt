...
enum class Wonder(override val effects: List<Effect>, override val cost: Cost) : Construction {
    CIRCUS_MAXIMUS(listOf(OpponentBuildingToDestroy(MANUFACTURE), Shield(1), VictoryPoints(3)), Cost(resources = mapOf(GLASS to 1, WOOD to 1, STONE to 2))),
    ...
    THE_MAUSOLEUM(listOf(DiscardedBuildingToBuild, VictoryPoints(2)), Cost(resources = mapOf(PAPYRUS to 1, GLASS to 2, CLAY to 2))),
    ...
    THE_STATUE_OF_ZEUS(listOf(OpponentBuildingToDestroy(RAW_MATERIAL), Shield(1), VictoryPoints(3)), Cost(resources = mapOf(PAPYRUS to 2, CLAY to 1, WOOD to 1, STONE to 1))),
